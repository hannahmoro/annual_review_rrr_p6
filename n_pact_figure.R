...
n_pact %>%
  ggplot(aes(x = year, y = estimate)) +
  geom_point(aes(color = journal)) +
  geom_line(data = rolling_averages, aes(x = year, y = rolling_estimate, color = journal, group = journal, se = FALSE)) +
  labs(x = "Year", y = "N-Pact Factor (Median N)", color = "Journal") +
  theme(axis.text.x = element_text(angle = -45, vjust = 0.5, hjust=1)) +
...
n_pact %>%
  ...
  geom_smooth(aes(color = research_area, group = research_area), se = FALSE, method = "loess") +
